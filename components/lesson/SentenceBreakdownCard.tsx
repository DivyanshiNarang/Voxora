...
export default function SentenceBreakdownCard({
  ...
  const [selectedWord, setSelectedWord] = useState<{
    type: "primaryText" | "pronunciation";
    index: number;
  } | null>(null);
  ...
  const playAudio = () => {
    ...
    const text = sentence.primaryText || sentence.pronunciation;
    if (!text) return;
  ...
  const renderInteractiveSentence = (type: "primaryText" | "pronunciation") => (
      ...
      <View style={styles.interactiveSentenceContainer}>
        {(sentence.words ?? []).map((word, index) => (
          <Pressable
            key={index}
            ref={(ref) => {
              if (type === "primaryText") hanziWordRefs.current[index] = ref;
          ...
          >
            <ThemedText
              style={[
                type === "primaryText" ? styles.hanziValue : styles.pinyinValue,
                selectedWord &&
          ...
          <View style={styles.breakdownItem}>
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              <ThemedText style={styles.label}>
                {PRONUNCIATION_LABEL[selectedLanguage]}:
              </ThemedText>
              <Pressable
                onPress={playAudio}
                disabled={disabled}
            ...
            {renderInteractiveSentence("pronunciation")}
          </View>
          <View style={styles.breakdownItem}>
            <ThemedText style={styles.label}>
              {SCRIPT_LABEL[selectedLanguage]}:
            </ThemedText>
            {renderInteractiveSentence("primaryText")}
          </View>
          <View style={styles.breakdownItem}>

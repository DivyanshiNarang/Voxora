...
import { getOptionPhrase, getQuestionPhrase } from "@/lib/phraseUtils";
...
interface StudyCard {
  key: string;
  word: Word;
  direction: "script-english" | "english-script";
}
...
const buildDeck = (words: Word[]): DeckBuckets => {
  const recognition: StudyCard[] = words.map((word) => ({
    key: `${word.primaryText}-recognition`,
    word,
    direction: "script-english",
  }));

  const recall: StudyCard[] = words.map((word) => ({
    key: `${word.primaryText}-recall`,
    word,
    direction: "english-script",
  }));
    ...
    questions.forEach((question) => {
      const wordSource =
        question.type === "listening_mc"
          ? getQuestionPhrase(question, selectedLanguage).words ?? []
          : question.options.flatMap(
              (opt) => getOptionPhrase(opt, selectedLanguage).words ?? [],
            );

      wordSource.forEach((word) => {
        if (word && word.primaryText && !allWords.has(word.primaryText)) {
          allWords.set(word.primaryText, word);
        }
      });
    });
